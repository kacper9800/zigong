<template>
  <div class="page" style="max-height: 40vh !important">
    ...
    <a-table :columns="columns" :data-source="categories" bordered>
      ...
      <a slot="actions" slot-scope="text">
        ...
        <img :src="require('../../assets/images/flags/ru.png')"
             @click="openEditModalWithTranslations(record, 'Ru')"/>
        <img :src="require('../../assets/images/flags/pl.png')"
    ...
    <a-modal :title='$t("categories.modal.addNewHeader")' :visible="addModalVisible"
      ...
      <div class="steps-content">
        <div v-if="modalCurrentStep === 0">
          <a-form id="categories-form" :form="form">
            <a-form-item :label='$t("categories.modal.name")'>
              <a-input type="text" v-decorator="['name', { rules: [{ required: true,
                       message: $t('categories.modal.namePlaceholder') }] }]"
                       :placeholder="$t('categories.modal.namePlaceholder')"/>
            </a-form-item>
            <a-form-item :label='$t("categories.modal.description")'>
              <a-input type="text" v-decorator="['description', { rules: [{ required: true,
                       message: $t('categories.modal.descriptionPlaceholder') }] }]"
                       :placeholder="$t('categories.modal.descriptionPlaceholder')"/>
            </a-form-item>
            <a-form-item :label='$t("categories.modal.homePageDescription")'>
              <a-input type="text" v-decorator="['homePageDescription', { rules: [{ required: true,
                       message: $t('categories.modal.homePageDescriptionPlaceholder') }] }]"
        ...
        <div v-if="modalCurrentStep === 1">
          <a-form id="categories-form">
            ...
            <a-form-item v-if="selectedOption === 1"
                         :label="$t('categories.modal.photoUpload.choosePhoto')">
              <div class="clearfix">
    ...
    <a-modal :title='$t("categories.modal.editHeader" + selectedLanguageEdition) + " "'
      ...
      <div class="steps-content">
        <div v-if="modalCurrentStep === 0">
          <a-form id="categories-form" :form="form">
            <a-form-item :label='$t("categories.modal.name")'>
              <a-input type="text" :placeholder="$t('categories.modal.namePlaceholder')" required/>
            </a-form-item>
            <a-form-item :label='$t("categories.modal.description")'>
              <a-input type="text" :placeholder="$t('categories.modal.descriptionPlaceholder')"
                       required/>
            </a-form-item>
            <a-form-item :label='$t("categories.modal.homePageDescription")'>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      openEditModalWithTranslations(record, code) {
        console.log(record);
